...
APP.controller.List = ClassAvanti.extend({
  ...
  searchResult() {
    ...
    const $resultBar = `<!-- START: Total products -->
                        <div class="search-bar__total-products">
                          <span class="total-products">${total}</span> produto${this._plural(total, "s")} encontrado${this._plural(total, "s")}
                        </div>
                        <!-- END: Total products -->`
  ...
  startAvantiSearch() {
    ...
    $resultItems.on("avantisearch.afterSearch", (event, options, request) => {
      ...
      APP.i.EnhancedEcommerce.handleProductImpressionsList(request.query.PageNumber)
