...
APP.controller.LpBuyTogetherList = ClassAvanti.extend({
  ...
  start() {
    ...
    sets.map((item, index) => this._getProducts(item.products, products => {
      let bestPriceSku = [];
      ...
      let availableSkus = bestPriceSku.filter(item => item.sellers[0].commertialOffer.AvailableQuantity > 0)
      if(availableSkus.length > 1) _self.postPrice(item, bestPriceSku)
    }));
  },
  ...
  getSkus(productPage, productResponse, callback) {
    ...
    callback(skusFiltered.sort((a, b) => a.sellers[0].commertialOffer.Price - b.sellers[0].commertialOffer.Price)[skusFiltered.length - 1]);
  },
  ...
  postPrice(products, skus) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        let itemValue = data.totals.find(item => item.id === "Items");
        itemValue = itemValue != undefined ? itemValue.value : 0;
  
        let discountsValue = data.totals.find(item => item.id === "Discounts");
        discountsValue = discountsValue != undefined ? discountsValue.value : 0;
  
        let best = (itemValue + discountsValue);
  
        let discount = discountsValue;
        
        let list = helpers._formatMoney(itemValue / 100, 2, ',', '.', 'R$');
        ...
        $('.section__products-list').append(templateProduct(products.name, imagem, list, helpers._formatMoney(best / 100, 2, ',', '.', 'R$'), bestInstallment, ids))
        
        if($('.section__products-list .product').length === sets.length -1){
          setTimeout(function(){
            $('.section__products-list').addClass('visible');
          }, 1000)
        }
      },
