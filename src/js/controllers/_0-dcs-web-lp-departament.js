    ...
    $.ajax({
      ...
      success: data => {
        for (const departament of data.CategoriesTrees) {
          if (this.departamentSlug === departament.Link.substr(1).toLowerCase()) {
    ...
    for (const category of categories) {
      const template = `<div class="product-categories">
                          <h2 class="product-categories__title">
                            <a href="${category.Link.toLowerCase()}" class="product-categories__title-link" title="${category.Name}">${category.Name}</a>
                          </h2>
                            ...
                            ${category.Children.map(children => `
                              <li class="product-categories__item">
                                <h3 class="product-categories__item-title">
                                  <a href="${children.Link.toLowerCase()}" class="product-categories__link">${children.Name}</a>
                                </h3>
                              </li>
                            `).join('')}
                          </ul>
