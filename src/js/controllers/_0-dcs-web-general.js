APP.controller.General = ClassAvanti.extend({
  ...
  setup() {
              ...
              ? `
                  <div class="blog-tips">
                    <div class="blog-tips__img">
                      <a href="${item.link}" class="blog-tips__img-link" title="${item.title.rendered}">
                        <img src="${
                          item.better_featured_image === null
                            ? ""
                            : item.better_featured_image.media_details.sizes["post-thumbnail"].source_url
                        }" alt="${
                  item.better_featured_image === null ? item.title.rendered : item.better_featured_image.alt_text
                }" />
                      </a>
                    </div>
                    ...
                    <div class="blog-tips__content">
                      <h3 class="blog-tips__title">
                        <a href="${item.link}" class="blog-tips__title-link" title="${item.title.rendered}">${
                  item.title.rendered
                }</a>
                      </h3>
                      <h4 class="blog-tips__subtitle">
                        <a href="${item.link}" class="blog-tips__subtitle-link" title="${item.excerpt.rendered}">${
    ...
    APP.i.BlogEventsEntries = new APP.component.BlogEntries({
      url: 'https://souesportista.decathlon.com.br/wp-json/eventos_order/v1/'+ typeBlogCall +'?slug=',
  ...
  bindScrollOptions() {
    const { $avalieFixed, $mainHeader, $mainTopHeader } = this.options
  ...
  bindBreadCrumb() {
    ...
    if (breadCrumb.find("ul li:first").find("i.bread-crumb-icon--home").length == 0) {
      breadCrumb.find("ul li:first").prepend('<i class="bread-crumb-icon--home"></i>')
    }
  },
    ...
    $.ajax({
      ...
      .done(function(res) {
        ...
        if (res.IsUserDefined == true) {
          nameUserLogged = nameUserLogged == null ? "Esportista" : nameUserLogged

          $("#welcomeNameUserDtp").html(`Olá ` + nameUserLogged + `, seja bem-vindo`)
          $(".account-user-name").html(
            `Olá <span class="link-blue">` + nameUserLogged + `</span>!<a href="/minha-conta">Acesse sua conta</a>`
          )
          $(".header__content--user").removeClass("not-logged").addClass("logged")
  ...
  bindLogoutButtons() {
    $(document).on("click", 'a[href="/no-cache/user/logout"], a[href="/logout"]', event => {
      event.preventDefault()
