...
APP.component.EnhancedEcommerce = ClassAvanti.extend({
  ...
  handleProductImpressions: function() {
    ...
    $('.shelf-item').each(function() {
      if(nameList !== $(this).closest('.main-shelf').find('h2').text()) {
        idx = 0
        nameList =  $(this).closest('.main-shelf').find('h2').text();
      }
      idx ++
   
      if(queryString !== '' && queryString !== undefined){
        nameCollection = pageInfo[0].pageCategory + ' ' + decodeURIComponent(queryString) + ' - ' + nameList
      } else {
        nameCollection = pageInfo[0].pageCategory + ' - ' + nameList
      }
      
      impressions.push(self.getProductByShelfItem($(this), idx, nameCollection))
    })

    this.handleDataLayerEventPush('productImpression', 'impressions', impressions)
  },
  ...
  handleProductImpressionsList: function(page) {
    ...
    if(pageCateg === "InternalSiteSearch") {
      ...
      nameList =  (depName === catName) ? pageCateg + ' - ' + depName : pageCateg + ' - ' + depName + ' > ' + catName
    }
       
    if((this.options.curPageList == 0 && page == 1) || this.options.curPageList !== page) {
      setTimeout(function() {
        $('.shelf-item').closest(`li[page="${page}"]`).each(function() {
          self.options.idxList ++
          impressions.push(self.getProductByShelfItem($(this).find('.shelf-item'), self.options.idxList, nameList))
        })
        
        if(impressions.length > 0) {
          self.handleDataLayerEventPush('productImpression', 'impressions', impressions)
        }
  ...
  handleProductClick: function() {
    ...
    $('body').on('click', '.shelf-item__img-link, .shelf-item__title-link', function(e) {
      e.preventDefault()

      const shelfItem = $(this).closest('.shelf-item')
      const nameList =  $(this).closest('.main-shelf').find('h2').text();
      const clickedProduct = self.getProductByShelfItem(shelfItem, shelfItem.parent('li').index() + 1, nameList)
  ...
  handleProductDetailView: function() {
    const product = this.getProductBySkuJson()
  ...
  handleCartItemChange: function() {
    ...
    $(window).on('orderFormUpdated.vtex', (e, orderForm) => {
      ...
      window.previousQuantity =
        typeof window.previousQuantity !== typeof undefined ? window.previousQuantity : currentQuantity
      window.previousItems = typeof window.previousItems !== typeof undefined ? window.previousItems : items
      ...
      if (currentQuantity != previousQuantity) {
        ...
        return hasAddedItem ? self.onAddCartItem(item) : self.onRemoveCartItem(item)
      }
  ...
  handlePromotionClick: function() {
    ...
    if (this.getUrlQueryStringExists(promo_name) && this.getUrlQueryStringExists(promo_id)) {
  ...
  handleProductSkuChange: function() {
    ...
    $('.sku-selector-container input, .sku-selector-container select').change(function() {
      const skuId = self.getParameterByName('sku', $('.buy-button-ref').attr('href'))

      if (!skuId) return true
      const item = skuJson.skus.filter(sku => sku.sku == skuId)

      if (!item.length) return true
      const productFields = self.pick(item[0], 'sku', 'skuname', 'bestPrice', 'skuname')
      const product = {
        id: productFields.sku,
        name: skuJson.name,
        category: categories,
        variant: productFields.skuname,
        price: productFields.bestPrice / 100,
      }
  ...
  handleGlobalVariables: function() {
    ...
    if (visitor.length) {
      const { visitorId: user_id, visitorExistingCustomer: returning, visitorContactInfo } = visitor[0]
  ...
  onAddCartItem: function(item) {
    ...
    product['category'] = item.productCategories ? item.productCategories[Object.keys(item.productCategories)[0]] : ''
   ...
   */
  getProductBySkuJson: function() {
    const categories = this.getProductPageCategories()
    
    const brandName = (dataLayer[0] !== undefined ? dataLayer[0].productBrandName : [])
    const { name, productId: id } = skuJson
    const { bestPrice: price, skuname: variant } = skuJson.skus[0]
    // const { productDepartmentName } = (dataLayer[0] !== undefined ? dataLayer[0] : [])
    // const { productCategoryName } = (dataLayer[0] !== undefined ? dataLayer[0] : [])
    // console.log('here', productCategoryName, productDepartmentName)
  ...
  getProductByShelfItem: (shelfItem, position, nameColection) => {
    return {
      ...
      category: shelfItem.data('product-department') + ' > '+ shelfItem.data('product-category'),
  ...
  getProductByOrderFormItem: function(item) {
    const product = this.pick(item, 'id', 'name', 'price', 'quantity', 'skuName')
  ...
  getUrlQueryStringExists: field => {
    ...
    return href.indexOf('?' + field + '=') != -1 || href.indexOf('&' + field + '=') != -1
  },
  ...
  getParameterByName: (name, url = window.location.href) => {
    ...
    return !results || !results[2] ? null : decodeURIComponent(results[2].replace(/\+/gi, ' '))
  },
   ...
   */
  handleDataLayerEventPush: (event = 'gtm.dom', action, content, eventCallback = null) => {
    dataLayer.push({
      event,
      ecommerce: {
        currencyCode: 'BRL',
