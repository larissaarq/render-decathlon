  ...
  window.AvantiSearch = {
    ...
    _createButtons: function () {
      ...
      $('.resultItemsWrapper div[id^=ResultItems]')
        ...
        self.options.$mainShelf.addClass(self.options.classMaskHide)
    },
    ...
    _hideButton: function (button) {
      ...
      if(button === self.options.classLoadMore){
        self.options.$mainShelf.removeClass(self.options.classMaskHide)
      }
    ...
    bindFilters: function () {
      ...
      self.options.$filters.find('input').on('change', function (event) {
        ...
        var name = _this.closest('fieldset').find('h5.Especificações').text()
        
        if (checked) {
          $label.addClass(self.options.classFilterActive);

          /*window.dataLayer = window.dataLayer || [];
          let gaEvent = 'Filtro ' + name;
          let gaCategory = 'Input Filtro';
          let gaLabel =  name +' - '+ $label.text(); */ 
          
          // dataLayer.push({'event': gaEvent,'category': gaCategory,'label': gaLabel });
          ga("send", "event", "Filtro "+name, "Input Filtro", name +' - '+ $label.text())
