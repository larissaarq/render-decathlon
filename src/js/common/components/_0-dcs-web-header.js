APP.component.Header = ClassAvanti.extend({
  ...
  setup() {
    ...
    this.options = {
      ...
      $classSlickNanoBarHeader: $("#header-alert .slick-frame")
    };
  },
  ...
  startNanoBarHeader() {
    const { $headerItems } = this.options;
    ...
    if($("#header-alert").length){
      ...
      //show alert
      if ($("#header-alert .alert-item").length > 0) {
        $("#header-alert").removeClass("hide");
        this.slickNanoBarHeader();
      }
    }
  },

  slickNanoBarHeader() {
    const { $classSlickNanoBarHeader } = this.options;

    const slickOptionsBar = {
      dots: false,
      infinite: true,
      autoplay: true,
      speed: 500
    ...
    $classSlickNanoBarHeader.not('.slick-initialized').slick(slickOptionsBar);
  ...
  showAlert() {
    const { $headerAlertContent } = this.options;
